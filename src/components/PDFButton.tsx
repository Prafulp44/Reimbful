...
const PDFButton: React.FC<PDFButtonProps> = ({ trip, variant = 'full', category }) => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // 2. Prepare groups
      const categories: ExpenseCategory[] = category 
        ? [category] 
      ...
      // 3. Generate PDFs
      const pdfGenerationPromises = categories.map(async (cat) => {
        const categoryExpenses = allExpenses.filter(e => e.category === cat);
        if (categoryExpenses.length === 0) {
          if (category) toast.error(`No expenses found for ${cat}`);
          return;
        }
        ...
        // Summary page
        partPromises.push(pdf(<TripSummaryPDF trip={trip} expenses={categoryExpenses} category={cat} />).toBlob().then(b => b.arrayBuffer()));
        ...
        if (finalBlob && finalBlob.size > 0) {
          const fileName = `${dateStr} ${cat}.pdf`;
      ...
      if (!category) {
        ...
        const hasExpenses = allExpenses.some(e => e.category === category);
        if (hasExpenses) {
          toast.success(`${category} PDF downloaded!`);
        }
      }
    } catch (error) {
    ...
    return (
      ...
      >
        {loading ? (
          ...
          <><FileText className="w-4 h-4" /> {category || 'Bills'}</>
        )}
      </button>
    );
  }
    ...
    <div className="flex-1">
      ...
      >
        {loading ? (
          ...
          <><FileText className="w-4 h-4" /> {category ? `${category} Bills` : 'All Bills'}</>
        )}
      </button>
    </div>
  );
